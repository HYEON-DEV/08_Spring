...
import com.hyeon.cookie_session.models.Member;
...
public class SessionController {
    ...
    public String home(Model model, HttpServletRequest request) {
        ...
        HttpSession session = request.getSession();
         ...
         /** 2) 세션값 가져오기
         * 값으로 가져오는 것은 Object 타입 => Unboxing 처리를 해야한다
    ...
    public String save(HttpServletRequest request,
        ...
        /**
         * 2) 세션값 저장하기
         * 값으로 저장하는 것은 Object 타입 => Boxing 처리가 일어난다
    ...
    public String loginOk(
        ...
        /** 1) 로그인 가능 여부 검사 */
        if (!userId.equals("hyeon") || !userPassword.equals("1234")) {

            /** 알림 메시지 표시 후 바로 이전 페이지로 이동 => Helper에 이식 예정
             * HTTP 403 Forbidden 클라이언트 오류 상태 응답 코드는 서버에 요청이 전달됐지만, 
             * 권한 때문에 거절되었다는 것을 의미
             */
        ...
        /** 2-1) request 객체를 사용해서 세션 객체 만들기 */
        HttpSession session = request.getSession();

        /** 2-2) 세션값 저장하기
         * 값으로 저장하는 것은 Object 타입 => Boxing 처리가 일어난다
         * 기본 데이터 타입 저장할 때는 Wrapper 클래스 사용
         */
